function z = FEM_1d_quadratic_Poisson(n)
...
re_gs = reshape(gs1_pt, 1, 1, gs1_n);
A_pre = [(4 * re_gs - 3) .^ 2, ...
            -(4 * re_gs - 3) .* (8 * re_gs - 4), ...
            (4 * re_gs - 1) .* (4 * re_gs - 3); ...
         -(4 * re_gs - 3) .* (8 * re_gs - 4), ...
...
ind = sort([1:2 * n + 1 3:2:2 * n]);
i = reshape(repmat(reshape(ind, 3, n), 3, 1), 3, 3 * n);
j = repmat(ind, 3, 1);
v = reshape(tensorprod(c_global .* uv_global, gs1_wt, 3, 2), 3, 3 * n);

A = sparse(i, j, v, 2 * n + 1, 2 * n + 1);

%% right_side
F_global = rhs(phy_gs);
F_global = F_global(reshape(ones(3, 1) * (1:n), 3 * n, 1), :);

v_global = repmat(u_pre, n, 1);

T = sparse(ind, 1:3 * n, ones(1, 3 * n), 2 * n + 1, 3 * n);
...
T = sparse(sort([1:n 1:n 1:n]), 1:3 * n, ones(1, 3 * n), n ,3 * n);
